...
//  static boolean JIKES = false;
	...
	private static String fileNameFromContext(Context context) {
		FileSplit fsFileSplit = (FileSplit) context.getInputSplit();
		String fileName = fsFileSplit.getPath().getName().toString();
		return fileName;
	}
...
//				untrustedMapperResult.set(i, Pair.create(key, mid.toString()));
	...
	public void map(Object someUnkownKey, Text currentLine, Context context) throws IOException, InterruptedException {
		...
		// System.out.println("Begin map key "+ someUnkownKey.toString()+ " line" + currentLine.toString());
		...
		} else if (dataset.compareToIgnoreCase("100Sales") == 0) {
			// TODO: Choose which columns we return to the untrusted mapper
			// Right now, we will provide full access
			kvPairs = getSalesRecordAnonymizedPairByProduct(currentLine.toString());
		} else if (dataset.compareToIgnoreCase("20newsgroup") == 0) {
			// TODO: Get the name of the document somewhere
			String fileName = fileNameFromContext(context);
			// System.out.println("[main] File name is " + fileName);
...
//	          DIFC.startMapInvocation(b);
		...
		try {
			...
			// TODO: Check how to get parameters from CLI
			List<String> configurationValues = getConfigurationForUntrustedMapper(context, dataset, mapperClassName);
			String[] configurationObject = new String[configurationValues.size()];
			configurationObject = configurationValues.toArray(configurationObject);
;			untrustedMapper.configure(configurationObject);
...
//			untrustedMapperResult = (Map<String, Double>) um.map(null, kvPairs);
			...
			for (String[] pair : kvPairs) {
				String key = pair[0];
				Object result = untrustedMapper.map(key, pair[1]);
				if (result != null) {
					String[] newKv = new String[2];
					newKv[0] = key;
					newKv[1] = (String) result;
					untrustedMapperResult.add(newKv);	
				} else {
					// System.out.println("Result for key -val was null, not using " + key + " " + pair[0]+ " " + pair[1]);
				}
			}
...
//	          DIFC.endMapInvocation(); 
	...
	private List<String> getConfigurationForUntrustedMapper(Context context, String dataset, String untrustedMapperClassName) {
		if (dataset.compareToIgnoreCase("20newsgroup") == 0) {
			// Read file from hadoop
			// TODO: Configure the path name
			Configuration conf = context.getConfiguration();
			Path path = new Path("/vocabulary.txt");
			List<String> tokens;
			try {
				FileSystem fileSystem = FileSystem.get(conf);
				FSDataInputStream in = fileSystem.open(path);
				tokens = new ArrayList<String>(10000);
				BufferedReader reader = new BufferedReader(new InputStreamReader(in));
				tokens = reader.lines().collect(Collectors.toList());
				// System.out.println("Read " + tokens.size() + " elements, the being " + tokens.get(0));
				reader.close();
				in.close();
				return tokens;
			} catch (IOException e) {
	...
	// so that it is available to the untrusted mapper
	// This needs to return ["ProductName", stringContainingOnlyNonConfidentialData]
	private static ArrayList<String[]> getSalesRecordAnonymizedPairByProduct(String inputLine) {
		// You receive externally (maybe as runtime parameters the key the queryer wants
		// and you analyze if you can actually emit that parameter as a key
		// (you won't let someone use 'Name' as a key. In this case we just use Product as a key
		ArrayList<String[]> result = new ArrayList();
